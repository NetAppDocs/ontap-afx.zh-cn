...
NOTE: 从 ONTAP 9.19.1 开始，SVM 管理员可以使用专用的 System Manager 仪表板监控和管理自己的 SVM。详细了解 https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager SVM 管理员仪表板"^]
...
AFX 在配置和使用集群时会强制执行几个限制，包括最小值和最大值。这些限制分为几个类别。


CAUTION: 在增加 AFX 存储系统的操作参数之前，您应该评估它可能如何影响您的应用程序以及整个集群。例如，虽然扩展控制器节点数量会增加整体容量，但也可能增加集群网络中的流量。了解这些配置更改的含义以及它们可能对您的环境产生的影响非常重要。

每个集群的控制器节点:: 每个 AFX 集群必须至少有四个节点。节点的最大数量因 ONTAP 版本而异；例如，9.19.1 RC 版本允许每个 AFX 集群最多 32 个节点。
存储容量:: 这是集群存储可用区 (SAZ) 中所有 SSD 磁盘的总容量。最大存储容量根据ONTAP版本而有所不同。
集群交换机:: 集群存储网络中至少需要两台交换机。允许的最大值是根据集群中控制器节点的总数来确定的。


您应该查看 NetApp Hardware Universe and Interoperability Matrix Tool 中提供的详细信息，以确定 AFX 集群的功能。另请参阅 AFX 发行说明部分中的link:../release-notes/changes-to-limits-defaults.html["对 ONTAP 限制和默认值的更改"]。
